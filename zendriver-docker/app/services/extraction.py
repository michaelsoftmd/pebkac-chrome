...
class UnifiedExtractionService:
    ...
    def _format_full(self, doc_dict: Dict, url: str) -> str:
        """Format extraction results with full content display."""
...
"""
    ...
    async def extract(self,
                     ...
                     format_style: str = "compact") -> Dict[str, Any]:
        """Main extraction method with configurable output formatting

        Args:
            format_style: Output format - "compact" (word-limited for LLM context),
                         "full" (complete content), or "structured" (metadata-rich)
        """
        ...
        if not selector and not xpath:
            ...
            if trafilatura_result and trafilatura_result.get("status") == "success":
                ...
                if format_style == "full" or format_style == "structured":
                    response['formatted_output'] = self._format_full(metadata, current_url)
                ...
                if format_style == "full" or format_style == "structured":
                    response['formatted_output'] = self._format_full(fallback_data, current_url)
                else:  # compact (default)
